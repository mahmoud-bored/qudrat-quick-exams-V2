<script lang="ts">
    ...
    onMount(() => {
        if(!($generalCurrentQuestionsMap && $generalCorrectQuestionsMap && $generalIncorrectQuestionsMap)) {
            goto('/quiz')
        }
    })
    ...
    onMount(() => {
        if(correctQuestionsPercentage < 60) {
            console.log((document.querySelector('.results-summary-general-percentage')));
